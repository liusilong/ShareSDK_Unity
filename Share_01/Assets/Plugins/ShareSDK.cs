...
namespace cn.sharesdk.unity3d
{
	...
	public class ShareSDK : MonoBehaviour 
	{
		...
		void OnGUI(){
			if(GUILayout.Button("Show Share Menu",GUILayout.Width(300),GUILayout.Height(100))){
				Hashtable content = new Hashtable();
				content["content"] = "this is a test string.";
			//	content["image"] = "/storage/sdcard0/DCIM/screenshot.png";
		...
		IEnumerator GetPicture(){  
			...
			Texture2D tex = new Texture2D(with,height,TextureFormat.RGB24,false);//参数mipmap设置为false，你不需要为这个图片添加mipmap
			yield return new WaitForEndOfFrame();//用到这一句代码是因为要等到相机渲染完图片才能读取相机渲染的图片
			tex.ReadPixels(new Rect(0,0,with,height),0,0); 
			tex.Apply();
